...
public class Message implements Parcelable {
    ...
    public String getReview() {
        String review = data.getString("c.r");

        if (review == null) {
            return null;
        }

        if (review.equals("")) { //if we received empty review string, we should review our app
            return CountlyMessaging.getContext().getPackageName();
        }
    ...
    public Intent getIntent(Context context, Class <? extends Activity> activityClass) {
        if (hasLink()) {
            ...
            return new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=" + getReview()));
        } else if (hasMessage()) {
